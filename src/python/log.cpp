 ...
 */
static void log_callback(DrJitLogLevel level, const char *msg) {
    ...
    bool err_out = level == DrJitLogLevel::Error || level == DrJitLogLevel::Warn;
...
void export_log(nb::module_ &m, PyModuleDef &pmd) {
    ...
    jit_set_log_level_stderr(DrJitLogLevel::Disable);
    jit_set_log_level_callback(DrJitLogLevel::Warn, log_callback);

    pmd.m_free = [](void *) {
        // Switch from the Python logger to standard stderr output
        jit_set_log_level_stderr(DrJitLogLevel::Warn);
        jit_set_log_level_callback(DrJitLogLevel::Disable, nullptr);
    };
    ...
    nb::enum_<DrJitLogLevel>(m, "DrJitLogLevel")
        .value("Disable", DrJitLogLevel::Disable)
        .value("Error", DrJitLogLevel::Error)
        .value("Warn", DrJitLogLevel::Warn)
        .value("Info", DrJitLogLevel::Info)
        .value("InfoSym", DrJitLogLevel::InfoSym)
        .value("Debug", DrJitLogLevel::Debug)
        .value("Trace", DrJitLogLevel::Trace);

    m.def("set_log_level", [](DrJitLogLevel level) {
        jit_set_log_level_callback(level, log_callback);
    });

    m.def("set_log_level", [](int level) {
        jit_set_log_level_callback((DrJitLogLevel) level, log_callback);
    });
